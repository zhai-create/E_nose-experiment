...
%     end
    ...
    switch type
        ...
        case 'CSLPP'
            % Compute LPP mapping
            if isempty(varargin), [mappedA, mapping] = CSLPP(A, no_dims, 12, 1, eig_impl);
            elseif length(varargin) == 1, [mappedA, mapping] = CSLPP(A, no_dims, varargin{1}, 12,1, eig_impl); 
            else [mappedA, mapping] = CSLPP(A, no_dims, varargin{1}, varargin{2}, eig_impl); end
            mapping.name = 'CSLPP';
        case 'ECSLPP_optim'
            % Compute LPP mapping
            if isempty(varargin), [mappedA, mapping] = ECSLPP_optim(A, no_dims, 12, 1, eig_impl);
            elseif length(varargin) == 1, [mappedA, mapping] = ECSLPP_optim(A, no_dims, varargin{1}, 12,1, eig_impl); 
            else [mappedA, mapping] = ECSLPP_optim(A, no_dims, varargin{1}, varargin{2}, eig_impl); end
            mapping.name = 'ECSLPP_optim';     
        case 'NPE'
            % Compute NPE mapping
