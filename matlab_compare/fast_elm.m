function [TrainingAccuracy, TestingAccuracy] = fast_elm(train_data, train_label, test_data, test_label, Elm_Type, NumberofHiddenNeurons, ActivationFunction,C)

% % train_data=RE_Source;
% % train_label=Source_label';
% % test_data= RE_Target;
% % test_label=Target_label';
% % Elm_Type=1;
% % NumberofHiddenNeurons=2000;
...
OutputWeight=pinv(eye(size(H,1))/C+H * H') * H * T'; %已修改  inv改成了pinv  % faster method 1 //refer to 2012 IEEE TSMC-B paper
...
%%%%%%%%%% Calculate training & testing classification accuracy
    ...
    for i = 1 : size(TV.T, 2)
        [x, label_index_expected]=max(TV.T(:,i));     % 期望输出的值
        [x, label_index_actual]=max(TY(:,i));         % 真实输出的类型
        if label_index_actual~=label_index_expected
